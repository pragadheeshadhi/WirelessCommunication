...
f2 = 2;             % Carrier frequency for bit 0
% Message signal
...
% Generate the FSK modulated signal
modulated_signal = zeros(1, length(t));
for i = 1:length(me)
    if me(i) == 1
        modulated_signal((i-1)*samples_per_bit + 1:i*samples_per_bit) = sin(2 * pi * f1 * t((i-1)*samples_per_bit + 1:i*samples_per_bit));
    else
        modulated_signal((i-1)*samples_per_bit + 1:i*samples_per_bit) = sin(2 * pi * f2 * t((i-1)*samples_per_bit + 1:i*samples_per_bit));
    end
end
...
for i = 1:length(me)
    ...
    power_f2 = mean(abs(hilbert(segment .* sin(2 * pi * f2 * t((i-1)*samples_per_bit + 1:i*samples_per_bit)))));
...
subplot(3, 1, 2);
plot(t, modulated_signal); % Plot the FSK modulated signal
